...
def calibrate_with_zhang_method(config: dict, img_file_list: list):
    ...
    #
    # world_points3d = create_3d_point_of_checker_corners(
    ...
    # V = np.zeros((2 * num_img_data, 6))  # V matrix in Eq. 9, which includes some elements from the homography matrix
    #
    # points2d = []  # 2D points in the image plane.
    ...
    #     points3d.append(world_points3d)
    #
    #     h, _ = cv.findHomography(srcPoints=world_points3d, dstPoints=detected_corners2d)
    #
    #     H[3 * idx:3 * (idx + 1), :] = h
    #
    #     V[2 * idx, :] = vij(h, 0, 1).T
